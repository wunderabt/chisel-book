...
Those \code{.elsewhen} branches can be of arbitrary length.
...
Below we consider a \myref{https://en.wikipedia.org/wiki/One-hot}{one-hot} decoder,
where exactly one bit of the output is one.
...
Note that even if we enumerate all possible input values, Chisel still requires us to assign a
...
works only as expected when the input signal is one-hot encoded. For all other input values, the output
